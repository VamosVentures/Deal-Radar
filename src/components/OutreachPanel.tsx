      ...
      {saved ? (
        ...
        <div className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="block font-mono text-[10px] uppercase tracking-wider text-slate-mid">
              Tone
              <select value={tone} onChange={(e) => setTone(e.target.value as OutreachTone)} className="mt-0.5 w-full rounded-[2px] border border-line bg-panel px-2 py-1.5 font-body text-xs normal-case">
          ...
          <div className="flex flex-wrap items-center gap-2 rounded-[2px] border border-line bg-paper px-3 py-2 text-[11px] text-slate-mid">
          ...
          {email && (
            <div className="space-y-3">
              {email.weakEvidence && (
                <div className="rounded-[2px] border border-marigold/50 bg-marigold-soft px-3 py-2 text-xs">
              ...
              {!email.weakEvidence && email.warnings.length > 0 && (
                <div className="rounded-[2px] bg-paper px-3 py-2 text-xs text-slate-mid">{email.warnings.join(' ')}</div>
              )}
              ...
              <label className="block font-mono text-[10px] uppercase tracking-wider text-slate-mid">
                ...
                <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={12} className="mt-0.5 w-full rounded-[2px] border border-line bg-panel px-2 py-1.5 font-body text-xs normal-case leading-relaxed" />
              </label>

              <div className="rounded-[2px] border border-line bg-paper px-3 py-2 text-xs">
...
function Chip({ ok, label }: { ok: boolean; label: string }) {
  return (
    <span className={`rounded-[2px] px-1.5 py-0.5 font-mono text-[10px] font-semibold ${ok ? 'bg-verde-soft text-verde' : 'bg-line text-slate-mid line-through'}`}>
      {label}
    </span>
  );
}
